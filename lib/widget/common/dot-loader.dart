...
class FlyroLoader extends StatelessWidget {
  final Color? color;
  final double size;
...
extension DotLoaderVariants on FlyroLoader {
  static FlyroLoader small({Color? color}) => FlyroLoader(
        size: 16,
        dotSize: 3,
        color: color,
      );

  static FlyroLoader medium({Color? color}) => FlyroLoader(
        size: 20,
        dotSize: 4,
        color: color,
      );

  static FlyroLoader large({Color? color}) => FlyroLoader(
        size: 24,
        dotSize: 5,
        color: color,
      );

  static FlyroLoader primary() => FlyroLoader(color: AppColors.primary);
  static FlyroLoader white() => FlyroLoader(color: Colors.white);
  static FlyroLoader error() => FlyroLoader(color: AppColors.error);
}